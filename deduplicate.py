  ...
  for t in atoms.iteritems():
    if t[0][:7] == "Thread$":
      ...
      while s is not None:
        ...
        if g is not None:
          b = atoms[g]
          thread_hash += event_hash_without_addr(t[1], b)
  ...
  for th in sorted(threads_with_hash.keys()):
    for t in threads_with_hash[th]:
      ...
      while s is not None:
        ...
        if g is not None:
          b = atoms[g]
          instance_hash += event_hash_without_addr(atoms[t], b)
          gaddr = follow_edge(b, "address")
          if gaddr is not None:
            addrmap.setdefault(atoms[gaddr], len(addrmap.keys()))
            instance_hash += "a" + str(addrmap[atoms[gaddr]])
        page = follow_edge(a, "page")
        if page is not None:
          addrmap.setdefault(atoms[page], len(addrmap.keys()))
          instance_hash += "a" + str(addrmap[atoms[page]])
        s = follow_po_edge(atoms, a)
...
def parse(instance):
  ...
  if instance_hash in instances:
    ...
    if total - last_message >= 500:
      sys.stdout.write("         (%d/%d) %s\n" % (unique, total, str(datetime.now())))
      last_message = total
  else:
    unique += 1
    last_message = total
    sys.stdout.write("New hash (%d/%d) %s: %s\n" % (unique, total, str(datetime.now()), instance_hash))
    sys.stdout.flush()
